...
class KnowledgeSearchService:
    ...
    def search(
        ...
        *,
        ...
        search_kwargs = {"limit": limit, "document_id": document_id}
        if partition_kind is not None:
            search_kwargs["partition_kind"] = partition_kind
        if partition_id is not None:
            search_kwargs["partition_id"] = partition_id
        batches = self.vector_store.search(vector, **search_kwargs)
        if not batches:
            return []
